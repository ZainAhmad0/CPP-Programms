...
int main()
{
    ...
    while(choice!=4)
    {
        ...
        if(choice==1)
        {
            int rollNum;
            string name;
            cout<<endl<<"Enter Roll Num : ";
            cin>>rollNum;
            cin.ignore();
            cout<<"Enter Name :  ";
            getline(cin,name);
            obj[objCount++].setInformation(rollNum,name);
            cout<<endl<<"Student Enrolled Successfully"<<endl<<endl;
        }
        else if(choice==2)
        {
            cout<<endl<<"CR Name : "<<classRepresentative.getName()<<endl;
            cout<<"CR Roll Numer : "<<classRepresentative.getRollNumber()<<endl;
            cout<<endl<<"GR : "<<girlRepresentative.getName()<<endl;
